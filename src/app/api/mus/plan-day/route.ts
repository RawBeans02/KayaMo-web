...
export async function POST(request: Request) {
  const auth = await requireUser(request, 'Sign in to plan with Lis.');
  if (!auth.ok) return auth.response;
  // The handler reserves the allowance itself, after the body has parsed, so
  // a malformed request does not spend a slot.
  const result = await handlePlanDay(auth.supabase, auth.user.id, await request.json().catch(() => null), {
