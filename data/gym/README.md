...
This folder lives at `data/gym/` (not under `apps/web`). Packages never import from apps.

- **CSV** in `csv/` is the editable master.
- `pnpm gym:kb:build` compiles it to `packages/features/src/gym/kb/compiled.json` for the web gym picker and consult.
- `pnpm gym:kb:check` fails if compiled.json drifted from the CSVs.
- SQLite / XLSX / JSONL are companion artifacts. The desktop app does not read them at runtime. After editing CSVs, rebuild compiled.json; regenerate sqlite/jsonl separately if you use them.

The SQLite / XLSX / JSONL are companion artifacts. The desktop app does not read them at runtime. After editing CSVs, rebuild compiled.json; regenerate sqlite/jsonl separately if you use them.

Consult may only emit `exercise_id` values from this package. It cannot invent lifts.

## App workflow (live session)

Canonical live-session behavior is in `APP_WORKFLOW.md` and `LIVE_SESSION_UX.md`, with tables in `csv/app_*.csv`.

The knowledge CSVs answer "what exercises exist." Runtime tables (`gym_session_items`, planned sets, rest timers, session events) live in IndexedDB via `@kayamo/offline` — Dexie-local, not a new Postgres sync surface.

Rules that must stay in code:

- Manual/locked items outrank AI fill. AI fills gaps; it does not rebuild the frame.
- The same exercise may appear twice as separate `plan_item_id`s.
- Planned targets and performed sets are stored separately. Completing `80×6` never rewrites a planned `80×8`.
- Rest timers belong to the completed set. Completing a set does not auto-complete the next one.
- Sequence is a queue, not a forced wizard.
- Pain/discomfort is a flag plus substitute options. Never diagnose.
