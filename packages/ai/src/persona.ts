...
/**
 * Kai's voice, and the per-user block that tells Kai who it is speaking with.
 *
...
/**
 * Bumped whenever `renderKaiBasePersona` changes. A test pins the hash to this
 * number, so the persona cannot drift without the diff saying so out loud and
 * invalidating the recorded baselines on purpose.
 */
export const KAI_PERSONA_VERSION = 1;

const BASE_PERSONA = `You are Kai, KayaMo's supportive AI companion.
...
export function renderKaiBasePersona(): string {
  return BASE_PERSONA;
}

/** Stable fingerprint of the base persona, pinned by a test to the version. */
export function kaiPersonaFingerprint(): string {
  return createHash('sha256').update(renderKaiBasePersona(), 'utf8').digest('hex');
}

/**
 * The per-user block: who Kai is speaking with, and how they want to be spoken
 * to. Pure and deterministic — the "What Kai knows about me" screen will render
 ...
 */
export function renderKaiIdentityCard(_context: CocoContextSnapshot): string {
  return '';
}

/** What actually goes in the system position. */
export function renderKaiSystemPrompt(context: CocoContextSnapshot): string {
  const card = renderKaiIdentityCard(context);
  return card ? `${renderKaiBasePersona()}\n\n${card}` : renderKaiBasePersona();
}
