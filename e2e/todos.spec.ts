...
test.describe('todos desk', () => {
  ...
  test('matches the day planner: capture, check-off, and wired Plan / Brain dump', async ({ page }) => {
    ...
    await page.getByLabel('Capture a task').fill('E2E check-off');
    await expect(page.getByRole('button', { name: 'Add' })).toBeEnabled();
    await page.getByRole('button', { name: 'Add' }).click();
    const title = page.getByRole('button', { name: /^E2E check-off/ });
